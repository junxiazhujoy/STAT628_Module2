...
for i in range(len(dat_night)):
    ...
    if 'craft beer' in dat_night[i]['text']:
        temp_list=dat_night[i]['text'].split()
        for i in range(len(temp_list)-3):
            if ((temp_list[i]=='craft') and (temp_list[i+1]=='beer') and (temp_list[i+2]=='is' or temp_list[i+2]=='was')) and (temp_list[i+3] in adj):
                adj_list.append(temp_list[i+3].lower())
            elif ((temp_list[i]=='craft') and (temp_list[i+1]=='beer')) or ((temp_list[i]=='NOT_craft') and (temp_list[i+1]=='NOT_beer')):
...
adj_set_craft = set(adj_list)

file=open('craft_beer_adj.txt','w') 
file.write(str(adj_set_craft)); 
file.close()
