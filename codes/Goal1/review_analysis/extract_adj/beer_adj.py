...
for i in range(len(dat_night)):
    ...
    if 'beer' in dat_night[i]['text']:
        temp_list=dat_night[i]['text'].split()
        for i in range(len(temp_list)-2):
            if ((temp_list[i]=='beer') and (temp_list[i+1]=='is' or temp_list[i+1]=='was')) and (temp_list[i+2] in adj):
                adj_list.append(temp_list[i+2].lower())
            elif (temp_list[i]=='beer'):
...
adj_set_beer = set(adj_list)

file=open('beer_adj.txt','w') 
file.write(str(adj_set_beer)); 
file.close()
