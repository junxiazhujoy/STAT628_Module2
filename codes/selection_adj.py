...
for i in range(len(dat_night)):
    ...
    if 'beer selection' in dat_night[i]['text']:
        temp_list=dat_night[i]['text'].split()
        for i in range(len(temp_list)-3):
            if ((temp_list[i]=='beer') and (temp_list[i+1]=='selection') and (temp_list[i+2]=='is' or temp_list[i+2]=='was')):
                adj_list.append(temp_list[i+3].lower())
            elif ((temp_list[i]=='beer') and (temp_list[i+1]=='selection')):
...
adj_set_selection = set(adj_list)

file=open('beer_selection_adj.txt','w') 
file.write(str(adj_set_selection)); 
file.close()
